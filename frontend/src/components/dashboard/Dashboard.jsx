...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        const response = await apiService.getProfile(Cookies.get('token'),Cookies.get('user'));
  ...
  return (
    <div className="dashboard-container">
      <nav className="dashboard-navbar">
        <span className="dashboard-navbar-greeting">
          Hello, {name}
        </span>
        <ul className="dashboard-navbar-links">
          <li className="dashboard-navbar-link">My Profile</li>
          <li className="dashboard-navbar-link">Place an Order</li>
          <li className="dashboard-navbar-link" onClick={handleLogout}>Logout</li>
        </ul>
      </nav>
      ...
      <div className="dashboard-card">
        <form className="dashboard-form">
          ...
          <select  className="dashboard-form-input chart-select" value={symbol} onClick={fetchSymbols} onChange={handleSymbolChange}>
          <option value="" className="dashboard-form-option">Select a symbol</option>
          {symbolsList.map((symbol, index) => (
            <option key={index} className="dashboard-form-option"  value={symbol.instrument_name}>
